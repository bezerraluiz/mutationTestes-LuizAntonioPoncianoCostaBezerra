...
describe('Utilitarios', () => {
  ...
  describe('Métodos de manipulação de string', () => {
    ...
    test('deve identificar palíndromo com case sensitivity', () => {
      // Testa que toLowerCase está sendo usado corretamente
      expect(utilitarios.ehPalindromo('Aba')).toBe(true);
      expect(utilitarios.ehPalindromo('ABA')).toBe(true);
      expect(utilitarios.ehPalindromo('AbA')).toBe(true);

      // String que só é palíndromo com toLowerCase, não com toUpperCase
      // devido a caracteres especiais ou unicode
      expect(utilitarios.ehPalindromo('A Santa at NASA')).toBe(true);
      expect(utilitarios.ehPalindromo('Was it a car or a cat I saw?')).toBe(true);
    });

    test('deve usar toLowerCase no processamento de palíndromo', () => {
      // Spy para garantir que toLowerCase é chamado
      const testString = 'AbA';
      const stringPrototype = String.prototype;
      const toLowerCaseSpy = jest.spyOn(stringPrototype, 'toLowerCase');

      const resultado = utilitarios.ehPalindromo(testString);
  ...
  describe('Métodos utilitários diversos', () => {
    ...
    test('deve gerar número aleatório com multiplicação correta', () => {
      // Testa múltiplas vezes para garantir que usa multiplicação, não divisão
      const max = 100;
      const amostras = [];

      // Mock Math.random para testar a operação correta
      const randomOriginal = Math.random;
      Math.random = jest.fn(() => 0.5);

      const resultado = utilitarios.gerarNumeroAleatorio(max);

      // Se usar multiplicação: floor(0.5 * 100) = 50
      // Se usar divisão: floor(0.5 / 100) = 0
